    ...
    <AdminLayout title="Admin|Post Management" content="Post Management">
      <PostDetailModal isVisible={showPostDetailModal} onClose={closePostModal} postID={postId} />
        <div className="p-4 ">
          ...
          <h1 className='font-sans font-extrabold text-xl'>Posts List</h1>
          </div>
            ...
            <tbody >
              {posts?.map((item) => (
                <tr data-te-toggle="tooltip" title="View post details" onClick={()=>{showPostDetail(item.id)}} key={item.id} className=" cursor-pointer bg-gray-100 text-center border-b-2" >
                  ...
                  <td className="px-4 py-2 ">
                  {item.is_blocked ? (
                  <button data-te-ripple-init data-te-ripple-color="light" className="bg-gray-500 rounded-md p-2 text-white font-bold hover:bg-green-600 relative" onClick={() => blockPost(item.id)}>Unblock</button>
                  ) : (
                  <button data-te-ripple-init data-te-ripple-color="light" className="bg-gray-500 rounded-md p-2 text-white font-bold hover:bg-red-600 relative" onClick={() => blockPost(item.id)}>Block</button>
                  )}       
                  </td>
                </tr>
               ))} 

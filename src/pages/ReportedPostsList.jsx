    ...
    <AdminLayout title="Admin|Post Management" content="Post Management">
      <PostDetailModal isVisible={showPostDetailModal} onClose={closePostModal} postID={postId} />
        ...
        <div className="p-4 ">
          ...
          <h1 className='font-sans font-extrabold text-xl uppercase'>Reported Posts </h1>
          </div>
          <table className="table-auto w-full border-collapse border border-gray-300 shadow-md">
            ...
            <tbody>
              {posts?.map((item) => (
                <tr data-te-toggle="tooltip" title="View post details" onClick={()=>{showPostDetail(item.id)}}  key={item.id} className="bg-gray-100 text-center border-b-2">
                  <td className=" px-4 py-2 ">{item.author.username}</td>
                  <td className=" flex justify-center px-4 py-2 "><img className='w-20  rounded-lg' alt={item.username} src={item.img}/></td>
                  ...
                  <td className="px-4 py-2 ">
                  {item.is_blocked ? (
                  <button className="bg-gray-500 rounded-md p-2 text-white font-bold hover:bg-red-600 relative" onClick={() => blockPost(item.id)}>Unlock</button>
                  ) : (
                  <button className="bg-gray-500 rounded-md p-2 text-white font-bold hover:bg-green-600 relative" onClick={() => blockPost(item.id)}>Block</button>
                  )}       
                  </td>
                </tr>
               ))} 
